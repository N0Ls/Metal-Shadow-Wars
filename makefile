...
ifeq ($(OS),Windows_NT)
    ...
    ifeq ($(shell uname),Darwin)
		...
		INC_FLAGS += -I/Library/Frameworks/SDL.framework/Headers -I/Library/Frameworks/SDL_image.framework/Headers

		LDFLAGS += `sdl-config --libs` `sdl-config --cflags`

		LDFLAGS += -framework Cocoa -framework OpenGL -lSDL_image -lglut
...
LDFLAGS += $(SDL_FLAGS) $(GL_FLAGS) -l$(SDL)_image
